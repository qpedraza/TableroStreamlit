...
def preprocesar(texto):
    """
    Usa spaCy para lematización real en español.
    "baterías" → "batería", "pantallas" → "pantalla"
    Elimina stopwords, puntuación y palabras cortas.
    """
...
def analizar_con_huggingface(texto, token):
    ...
    if isinstance(result, list) and len(result) > 0:
        ...
        # cardiffnlp devuelve: Positive, Negative, Neutral
        mapa = {
...
#Sección 5: Porcentaje de sentimientos
st.header("📊 Porcentaje de Sentimientos")

# Clasificación simple basada en estrellas
def clasificar_por_estrellas(estrellas):
    if estrellas >= 4:
        return "😊 Positivo"
    elif estrellas == 3:
        return "😐 Neutro"
...
df_filtrado["sentimiento_simple"] = df_filtrado["estrellas"].apply(clasificar_por_estrellas)
conteo_sent = df_filtrado["sentimiento_simple"].value_counts()

colores_sent = []
for label in conteo_sent.index:
    if "Positivo" in label:
        colores_sent.append("#2ecc71")
    elif "Negativo" in label:
        colores_sent.append("#e74c3c")
    else:
        colores_sent.append("#f1c40f")

fig5, ax5 = plt.subplots(figsize=(6, 5))
wedges, texts, autotexts = ax5.pie(
    conteo_sent.values,
    labels=conteo_sent.index,
    colors=colores_sent,
    autopct="%1.1f%%",
    startangle=90,
    pctdistance=0.75
...
ax5.set_title("Distribución de sentimientos", color="#00d4ff", fontsize=13, fontweight="bold")
plt.tight_layout()
st.pyplot(fig5)
plt.close(fig5)
...
positivos = sum(1 for s in df_filtrado["sentimiento_simple"] if "Positivo" in s)
neutros   = sum(1 for s in df_filtrado["sentimiento_simple"] if "Neutro" in s)
negativos = sum(1 for s in df_filtrado["sentimiento_simple"] if "Negativo" in s)
m1.metric("😊 Positivas", positivos, f"{positivos/total*100:.1f}%")
m2.metric("😐 Neutras",   neutros,   f"{neutros/total*100:.1f}%")
m3.metric("😠 Negativas", negativos, f"{negativos/total*100:.1f}%")

#Explorador de opiniones
st.header("🔍 Explorador de Opiniones")

for i, row in df_filtrado.iterrows():
    estrellas = "⭐" * int(row["estrellas"])
    with st.expander(f"{estrellas} — {row['producto']}"):
